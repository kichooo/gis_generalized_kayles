...
public class GraphTests {
	...
	public void testGame() {
		Graph graph = new Graph(30, 100);
		Assert.assertFalse(graph.isGameOver());
		while (!graph.isGameOver()) {
			Vertex v = graph.makeDecision(1, 1, 1);
			System.out.println("Removing vertex " + v.toString());
			graph.kaylesRemove(v);
		}
		Assert.assertTrue(graph.isGameOver());
